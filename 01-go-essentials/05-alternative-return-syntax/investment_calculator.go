...
func calculateFutureValues(investmentAmount float64, expectedReturnRate float64, years float64) (futureValue float64, realFutureValue float64) { // 這裡的(float64, float64)代表回傳兩個float64 的futureValue, realFutureValue值
	futureValue = investmentAmount * math.Pow(1+expectedReturnRate/100, years)
	realFutureValue = futureValue / math.Pow(1+inflationRate/100, years)

	//return futureValue, realFutureValue
	return // 這裡不用寫回傳值，因為已經在func宣告回傳值
}
